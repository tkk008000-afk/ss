// ============================================================
// البوت المتكامل - النسخة النهائية مع نظام العملات والموقع
// ============================================================
...
app.get('/api/top-credits/:guildId', async (req, res) => {
  try {
    const { guildId } = req.params;
    const top = await User.find({ guildId }).sort({ credits: -1 }).limit(10);
    const guild = client.guilds.cache.get(guildId);
    const result = await Promise.all(top.map(async (u) => {
...
app.get('/', (req, res) => {
  res.send(`
    ...
    <head>
      ...
      <style>
        body { font-family: Arial, sans-serif; background: #2b2d31; color: #e0e0e0; padding: 20px; }
        .container { max-width: 800px; margin: auto; text-align: center; }
        .status { background: #1e1e22; padding: 20px; border-radius: 10px; margin: 10px 0; }
        h1 { color: #5865f2; }
        .stats { display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; margin: 20px 0; }
        .stat-card { background: #1e1e22; padding: 15px; border-radius: 8px; }
        .stat-card h3 { margin: 0; color: #aaa; }
        .stat-card p { font-size: 24px; margin: 5px 0; color: #fff; }
        .green { color: #57f287; }
        .blue { color: #5865f2; }
        .orange { color: #faa81a; }
        .purple { color: #9b59b6; }
    ...
    <body>
      <div class="container">
        ...
        <div class="stats" id="stats">
          <div class="stat-card"><h3>السيرفرات</h3><p id="guilds">...</p></div>
          <div class="stat-card"><h3>الأعضاء</h3><p id="members">...</p></div>
          <div class="stat-card"><h3>سرعة الاستجابة</h3><p id="ping">...</p></div>
          <div class="stat-card"><h3>وقت التشغيل</h3><p id="uptime">...</p></div>
        </div>
        <div class="status">
          <h3>🏆 أعلى 10 أعضاء في العملات</h3>
          <div id="topCredits">جاري التحميل...</div>
        </div>
      </div>
      <script>
        ...
        async function fetchTopCredits() {
          try {
            const guildId = window.location.pathname.split('/').pop() || '';
            const res = await fetch('/api/top-credits/' + guildId);
            const data = await res.json();
            let html = '<ol>';
            data.forEach((u, i) => {
              html += '<li>' + u.username + ' - ' + u.credits + ' 🪙</li>';
            });
            html += '</ol>';
            document.getElementById('topCredits').innerHTML = html || 'لا توجد بيانات';
          } catch(e) { console.error(e); }
        }
        fetchStats();
        fetchTopCredits();
...
// تعديل نموذج المستخدم لإضافة العملات
const UserSchema = new mongoose.Schema({
  ...
  credits: { type: Number, default: 0 }, // نظام العملات الجديد
  lastDaily: { type: Date, default: null }, // آخر مرة أخذ فيها المكافأة اليومية
...
const StoreItemSchema = new mongoose.Schema({
  guildId: String,
  roleId: String,
  price: Number, // السعر الآن بالعملات (Credits)
  description: String,
});
...
async function getMapConfig(guildId) {
  ...
  if (!map) {
    map = new MapConfig({ guildId });
    await map.save();
  }
  return map;
}
async function updateMapConfig(guildId, data) {
  ...
  return {
    'general': {
      ...
      fields: [
        ...
        { name: 'رصيد', value: 'عرض رصيد العملات الخاص بك', inline: true },
        { name: 'تحويل', value: 'تحويل عملات إلى عضو آخر', inline: true },
        { name: 'يومي', value: 'الحصول على مكافأة يومية', inline: true },
      ]
    },
    'admin': {
      ...
      fields: [
        ...
        { name: 'قائمة_المتحكمين', value: 'عرض قائمة المتحكمين (بما فيهم رتبة التحكم)', inline: false },
        ...
        { name: 'اعطاء_عملات / سحب_عملات', value: 'إدارة عملات الأعضاء (للمالك)', inline: false },
      ]
    },
    'levels': {
    ...
    'store': {
      title: '🛒 المتجر',
      description: 'شراء وبيع الرتب بالعملات.',
      fields: [
        { name: 'متجر', value: 'فتح المتجر لشراء الرتب (بالعملات)', inline: false },
...
function getSettingsData() {
  return {
    ...
    'autoline': {
      ...
      description: 'رسائل أو صور تظهر تلقائياً في قناة (فقط من الأعضاء العاديين).',
      options: [
    ...
    'store': {
      ...
      options: [
        '`اضافة_منتج @رتبة [السعر] [الوصف]` - إضافة منتج (بالعملات)',
...
async function generateServerMapEmbed(guild, member, config, mapConfig) {
  const embed = new EmbedBuilder()
    ...
    .setDescription(mapConfig.description || 'اضغط على أحد الأزرار أدناه لعرض القنوات التابعة لكل قسم.')
    .setColor(0x2b2d31)
    .setTimestamp()
...
client.on('messageCreate', async (message) => {
  ...
  try {
    ...
    // نظام العملات - ربح عملات عند إرسال الرسائل (مكافأة عشوائية)
    const creditGain = Math.floor(Math.random() * 5) + 1; // من 1 إلى 5 عملات
  ...
  // ===== الأوتو لاين (فقط للأعضاء العاديين، وليس البوت) =====
  // تم التعديل: الشرط message.author.bot يمنع البوت من تشغيل الأوتو لاين
  ...
  if (interaction.isCommand()) {
    ...
    if (commandName === 'رصيد') {
      ...
      const embed = new EmbedBuilder()
        ...
        .setTimestamp();
      if (getGeneralImage(interaction.guild, config)) embed.setImage(getGeneralImage(interaction.guild, config));
      await interaction.reply({ embeds: [embed], flags: MessageFlags.Ephemeral });
      return;
    }

    // ===== أمر يومي (مكافأة يومية) =====
    if (commandName === 'يومي') {
      const user = await getUser(guildId, interaction.user.id);
      const now = new Date();
      const dailyAmount = 100; // المكافأة اليومية
      ...
      if (user.lastDaily) {
        ...
        if (hoursDiff < 24) {
          ...
          return interaction.reply({ content: `⏳ يمكنك الحصول على مكافأتك اليومية بعد ${remaining} ساعة.`, flags: MessageFlags.Ephemeral });
        }
      }
    ...
    if (commandName === 'تحويل') {
      ...
      if (!target) return interaction.reply({ content: '⚠️ العضو غير موجود.', flags: MessageFlags.Ephemeral });
      if (target.id === interaction.user.id) return interaction.reply({ content: '❌ لا يمكنك التحويل لنفسك.', flags: MessageFlags.Ephemeral });
      if (amount < 1) return interaction.reply({ content: '⚠️ أدخل مبلغاً موجباً.', flags: MessageFlags.Ephemeral });
      
      const senderCredits = await getCredits(guildId, interaction.user.id);
      if (senderCredits < amount) {
        return interaction.reply({ content: `❌ ليس لديك رصيد كافٍ. رصيدك: **${senderCredits}** 🪙`, flags: MessageFlags.Ephemeral });
      }
      
      await removeCredits(guildId, interaction.user.id, amount);
    ...
    // ===== أمر قائمة_المتحكمين المعدل =====
    if (commandName === 'قائمة_المتحكمين') {
      const controllers = await getControllers(guildId);
      const config = await getGuildConfig(guildId);
      let allControllers = [...controllers];
      
      // إضافة الأعضاء الذين لديهم رتبة التحكم بالبوت
      if (config.botControllerRole) {
        const role = interaction.guild.roles.cache.get(config.botControllerRole);
        if (role) {
          const membersWithRole = role.members.map(m => m.id);
          for (const id of membersWithRole) {
            if (!allControllers.includes(id)) {
              allControllers.push(id);
            }
          }
        }
      }
      ...
      if (!allControllers.length) {
        return interaction.reply({ content: '📋 لا يوجد متحكمون.', flags: MessageFlags.Ephemeral });
      }
      
      const list = allControllers.map(id => `<@${id}>`).join('\n');
      const embed = new EmbedBuilder()
        .setTitle('🛡️ قائمة المتحكمين')
        .setColor(0x2b2d31)
        .setDescription(list)
        .setFooter({ text: `عدد المتحكمين: ${allControllers.length}` });
      await interaction.reply({ embeds: [embed], flags: MessageFlags.Ephemeral });
      return;
    }
    ...
    if (commandName === 'بانل') {
      ...
      const embed = await createTicketPanelEmbed(interaction.guild, config);
...
**صورة البانل:** ${config.uiTicketImage ? `[رابط](${config.uiTicketImage})` : 'غير محدد (سيتم استخدام الصورة الافتراضية)'}
**بانر عام:** ${config.uiBannerUrl ? `[رابط](${config.uiBannerUrl})` : 'غير محدد'}
        `)
        .addFields(
          { name: '📌 الأقسام', value: settings.sections.map((s, i) => `${i+1}. ${s.emoji || '📌'} **${s.name}** ${s.roleId ? `<@&${s.roleId}>` : '(بدون دور)'}${s.canRestart ? ' 🔄' : ''}`).join('\n') || 'لا يوجد أقسام' }
        );
      if (config.uiTicketImage) embed.setImage(config.uiTicketImage);
      const generalImage = getGeneralImage(interaction.guild, config);
      if (generalImage) embed.setThumbnail(generalImage);
      await interaction.reply({ embeds: [embed], flags: MessageFlags.Ephemeral });
      return;
    ...
    // ===== أمر لوحة_المهام المعدل =====
    if (commandName === 'لوحة_المهام') {
      // التحقق من صلاحية المتحكم أو رتبة التحكم بالبوت
      if (!(await hasPermission(interaction.member, guildId))) {
        return interaction.reply({ content: '❌ هذا الأمر للمتحكمين أو لرتبة التحكم بالبوت فقط.', flags: MessageFlags.Ephemeral });
      }
      const embed = new EmbedBuilder()
  ...
  if (interaction.isButton()) {
    ...
    // ===== أزرار المتجر تم إلغاؤها لأن الشراء يتم مباشرة من القائمة =====
    // تم حذف معالج store_approve و store_reject لأن الشراء أصبح فورياً
...
client.on('messageCreate', async (message) => {
  ...
  try {
      ...
      const settingHandlers = {
        ...
        'تذكرة_صورة': async () => {
          if (!value || !value.match(/^https?:\/\/.+/)) {
            return message.reply('⚠️ رابط صورة غير صالح. تأكد من أن الرابط يبدأ بـ http:// أو https:// وينتهي بامتداد صورة.');
          }
          await updateGuildConfig(guildId, { uiTicketImage: value });
          await message.reply(`✅ تم تعيين صورة لوحة التذاكر:\n${value}`);
        },
        'تذكرة_نص': async () => {
        ...
        'تذكرة_عنوان': async () => {
          ...
          await message.reply(`✅ تم تغيير عنوان لوحة التذاكر إلى:\n${value}`);
        },
        'تذكرة_اضافة': async () => {
        ...
        'ترحيب': async () => {
          ...
          await message.reply(`✅ تم تعيين قناة الترحيب: ${channel}`);
        },
        'رسالة_ترحيب': async () => {
          if (!value) return message.reply('⚠️ اكتب نص الترحيب.');
        ...
        'عنوان_ترحيب': async () => {
          if (!value) return message.reply('⚠️ اكتب عنوان الترحيب.');
          await updateGuildConfig(guildId, { welcomeTitle: value });
          await message.reply(`✅ تم تعيين عنوان الترحيب: ${value}`);
        },
        'خلفية_ترحيب': async () => {
          if (!value) return message.reply('⚠️ أدخل لون أو رابط صورة.');
        ...
        'سجلات': async () => {
          ...
          await message.reply(`✅ تم تعيين قناة السجلات: ${channel}`);
        },
        'قناة_سجلات_تذاكر': async () => {
          const channel = message.mentions.channels.first();
          if (!channel) return message.reply('⚠️ منشن القناة.');
          await updateGuildConfig(guildId, { ticketLogChannel: channel.id });
          await message.reply(`✅ تم تعيين قناة سجلات التذاكر: ${channel}`);
        },
        ...
        'اوتر_لاين': async () => {
          ...
          const text = value.replace(/<#\d+>/, '').trim();
          if (!text) return message.reply('⚠️ أدخل نص الأوتو لاين.');
          await setAutoLine(guildId, channel.id, { text, enabled: true });
          await message.reply(`✅ تم تعيين نص الأوتو لاين في ${channel}`);
        },
        'صورة_اوترلاين': async () => {
          const channel = message.mentions.channels.first();
          if (!channel || !value) return message.reply('⚠️ الصيغة: `!تعيين صورة_اوترلاين #قناة رابط`');
          const image = value.replace(/<#\d+>/, '').trim();
          if (!image.match(/^https?:\/\/.+/)) return message.reply('⚠️ رابط صورة غير صالح.');
          await setAutoLine(guildId, channel.id, { image, enabled: true });
          await message.reply(`✅ تم تعيين صورة الأوتو لاين في ${channel}`);
        },
        'تفعيل_اوترلاين': async () => {
          const channel = message.mentions.channels.first();
          if (!channel) return message.reply('⚠️ منشن القناة.');
          const auto = await getAutoLine(guildId, channel.id);
          auto.enabled = true;
          await auto.save();
        ...
        'اضافة_رتبة': async () => {
          ...
          let roles = config.uiRolesOptions || [];
          if (roles.includes(value)) return message.reply(`⚠️ الرتبة "${value}" موجودة بالفعل.`);
          roles.push(value);
          await updateGuildConfig(guildId, { uiRolesOptions: roles });
        ...
        'صورة_رتب': async () => {
          ...
          await message.reply(`✅ تم تعيين صورة الرتب: ${value}`);
        },
        'عنوان_قائمة_الرتب': async () => {
          if (!value) return message.reply('⚠️ أدخل النص.');
        ...
        'قناة_اقتراح': async () => {
          ...
          await message.reply(`✅ تم تعيين قناة الاقتراحات: ${channel}`);
        },
        'عنوان_اقتراح': async () => {
          if (!value) return message.reply('⚠️ أدخل النص.');
        ...
        'وصف_اقتراح': async () => {
          ...
          await message.reply(`✅ تم تعيين وصف الاقتراحات: ${value}`);
        },
        'لون_اقتراح': async () => {
          if (!value.match(/^#[0-9a-fA-F]{6}$/)) return message.reply('⚠️ أدخل لون بصيغة #هيكس.');
        ...
        'عنوان_اقتراح_بانل': async () => {
          if (!value) return message.reply('⚠️ أدخل النص.');
          await updateGuildConfig(guildId, { uiSuggestTitle: value });
          await message.reply(`✅ تم تعيين عنوان لوحة الاقتراحات: ${value}`);
        },
        'وصف_اقتراح_بانل': async () => {
          if (!value) return message.reply('⚠️ أدخل النص.');
          await updateGuildConfig(guildId, { uiSuggestDescription: value });
          await message.reply(`✅ تم تعيين وصف لوحة الاقتراحات: ${value}`);
        },
        'صورة_اقتراح_بانل': async () => {
          if (!value || !value.match(/^https?:\/\/.+/)) return message.reply('⚠️ رابط صورة غير صالح.');
          await updateGuildConfig(guildId, { uiSuggestImage: value });
          await message.reply(`✅ تم تعيين صورة لوحة الاقتراحات: ${value}`);
        },
        'بانر_اقتراح_بانل': async () => {
          if (!value || !value.match(/^https?:\/\/.+/)) return message.reply('⚠️ رابط صورة غير صالح.');
        ...
        'قناة_الاجازات': async () => {
          ...
          await message.reply(`✅ تم تعيين قناة طلبات الإجازات: ${channel}`);
        },
        'قناة_المودات': async () => {
          await message.reply('⚠️ قناة المودات غير محددة في النظام الحالي.');
        },
        'قناة_المتجر': async () => {
          const channel = message.mentions.channels.first();
          if (!channel) return message.reply('⚠️ منشن القناة.');
        ...
        'اضافة_منتج': async () => {
          const role = message.mentions.roles.first();
          if (!role) return message.reply('⚠️ منشن الرتبة.');
          const parts = value.replace(/<@&\d+>/, '').trim().split(/\s+/);
          if (parts.length < 2) return message.reply('⚠️ الصيغة: `!تعيين اضافة_منتج @رتبة السعر الوصف`');
          const price = parseInt(parts[0]);
          if (isNaN(price) || price < 1) return message.reply('⚠️ السعر غير صحيح.');
          const desc = parts.slice(1).join(' ') || 'لا يوجد وصف';
          await addStoreItem(guildId, role.id, price, desc);
          await message.reply(`✅ تم إضافة المنتج **${role.name}** بسعر **${price}** 🪙.`);
        },
        'حذف_منتج': async () => {
          if (!value) return message.reply('⚠️ أدخل معرف المنتج.');
          const deleted = await removeStoreItem(guildId, value);
          if (!deleted) return message.reply('❌ المنتج غير موجود.');
        ...
        'عنوان_متجر_بانل': async () => {
          if (!value) return message.reply('⚠️ أدخل النص.');
          await updateGuildConfig(guildId, { uiStoreTitle: value });
          await message.reply(`✅ تم تعيين عنوان لوحة المتجر: ${value}`);
        },
        'وصف_متجر_بانل': async () => {
          if (!value) return message.reply('⚠️ أدخل النص.');
          await updateGuildConfig(guildId, { uiStoreDescription: value });
          await message.reply(`✅ تم تعيين وصف لوحة المتجر: ${value}`);
        },
        'صورة_متجر_بانل': async () => {
          if (!value || !value.match(/^https?:\/\/.+/)) return message.reply('⚠️ رابط صورة غير صالح.');
        ...
        'عنوان_الاسم': async () => {
          ...
          await message.reply(`✅ تم تعيين عنوان لوحة تغيير الاسم: ${value}`);
        },
        'وصف_الاسم': async () => {
          if (!value) return message.reply('⚠️ أدخل النص.');
          await updateGuildConfig(guildId, { uiDescription: value });
          await message.reply(`✅ تم تعيين وصف لوحة تغيير الاسم: ${value}`);
        },
        'ملاحظة_الاسم': async () => {
          if (!value) return message.reply('⚠️ أدخل النص.');
          await updateGuildConfig(guildId, { uiNoteText: value });
          await message.reply(`✅ تم تعيين ملاحظة لوحة تغيير الاسم: ${value}`);
        },
        'بانر_الاسم': async () => {
          if (!value || !value.match(/^https?:\/\/.+/)) return message.reply('⚠️ رابط صورة غير صالح.');
          await updateGuildConfig(guildId, { uiBannerUrl: value });
          await message.reply(`✅ تم تعيين بانر لوحة تغيير الاسم: ${value}`);
        },
        'عنوان_اجازة_بانل': async () => {
          if (!value) return message.reply('⚠️ أدخل النص.');
          await updateGuildConfig(guildId, { uiLeaveTitle: value });
          await message.reply(`✅ تم تعيين عنوان لوحة الإجازات: ${value}`);
        },
        'وصف_اجازة_بانل': async () => {
          if (!value) return message.reply('⚠️ أدخل النص.');
          await updateGuildConfig(guildId, { uiLeaveDescription: value });
          await message.reply(`✅ تم تعيين وصف لوحة الإجازات: ${value}`);
        },
        'صورة_اجازة_بانل': async () => {
          if (!value || !value.match(/^https?:\/\/.+/)) return message.reply('⚠️ رابط صورة غير صالح.');
        ...
        'نقاط_المهمة': async () => {
          ...
          await updateGuildConfig(guildId, { pointsPerTask: points });
          await message.reply(`✅ تم تعيين نقاط المهمة الافتراضية: ${points}`);
        },
        'نقاط_الترقية': async () => {
          const points = parseInt(value);
          if (isNaN(points) || points < 1) return message.reply('⚠️ أدخل عدداً صحيحاً موجباً.');
        ...
        'عنوان_خريطة': async () => {
          if (!value) return message.reply('⚠️ أدخل النص.');
          const mapConfig = await getMapConfig(guildId);
          mapConfig.title = value;
          await mapConfig.save();
          await message.reply(`✅ تم تعيين عنوان الخريطة: ${value}`);
        },
        'وصف_خريطة': async () => {
          if (!value) return message.reply('⚠️ أدخل النص.');
          const mapConfig = await getMapConfig(guildId);
          mapConfig.description = value;
          await mapConfig.save();
          await message.reply(`✅ تم تعيين وصف الخريطة: ${value}`);
        },
        'بانر_خريطة': async () => {
          if (!value || !value.match(/^https?:\/\/.+/)) return message.reply('⚠️ رابط صورة غير صالح.');
          const mapConfig = await getMapConfig(guildId);
          mapConfig.banner = value;
          await mapConfig.save();
          await message.reply(`✅ تم تعيين بانر الخريطة: ${value}`);
        }
      };

      const handler = settingHandlers[subCmd];
      if (handler) {
        if (message.author.id !== OWNER_ID) return message.reply('❌ هذا الأمر للمالك فقط.');
        await handler();
      } else {
    ...
    if (cmd === 'بانل') {
      ...
      const settings = await getTicketSettings(guildId);
      const embed = await createTicketPanelEmbed(message.guild, config);
...
**بانر عام:** ${config.uiBannerUrl ? `[رابط](${config.uiBannerUrl})` : 'غير محدد'}
    ...
    if (cmd === 'لوحة_المهام') {
      // التحقق من صلاحية المتحكم أو رتبة التحكم بالبوت
      if (!(await hasPermission(message.member, guildId))) {
        return message.reply('❌ هذا الأمر للمتحكمين أو لرتبة التحكم بالبوت فقط.');
      }
      const embed = new EmbedBuilder()
        .setTitle('📋 لوحة المهام الإدارية')
        .setDescription('اختر الإجراء المناسب من الأزرار أدناه.')
    ...
    if (cmd === 'اعطاء_عملات') {
      ...
      if (isNaN(amount) || amount < 1) return message.reply('⚠️ أدخل مبلغاً موجباً.');
      await addCredits(guildId, target.id, amount);
      await message.reply(`✅ تم إعطاء ${amount} 🪙 إلى ${target}`);
      return;
    }
    ...
    if (cmd === 'سحب_عملات') {
      ...
      if (!target) return message.reply('⚠️ منشن العضو.');
      if (isNaN(amount) || amount < 1) return message.reply('⚠️ أدخل مبلغاً موجباً.');
      const success = await removeCredits(guildId, target.id, amount);
      if (!success) return message.reply(`❌ ${target} ليس لديه رصيد كافٍ.`);
      await message.reply(`✅ تم سحب ${amount} 🪙 من ${target}`);
      return;
    }
    ...
    if (cmd === 'قائمة_المتحكمين') {
      ...
      if (config.botControllerRole) {
        const role = message.guild.roles.cache.get(config.botControllerRole);
        if (role) {
          const membersWithRole = role.members.map(m => m.id);
          for (const id of membersWithRole) {
            if (!allControllers.includes(id)) {
      ...
      if (!allControllers.length) {
        return message.reply('📋 لا يوجد متحكمون.');
      }
